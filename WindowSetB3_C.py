# -*- coding: utf-8 -*-
"""
Created on Thus Jan  25 09:12:18 2018
@author: JRCHAN
Description: 准则层B（售电风险）-准则层C主观权重设置
"""
import os
...
class WindowSetB3_C(QtWidgets.QTableWidget,QtWidgets.QWidget): 
    def __init__(self, parent=None): 
        super(WindowSetB3_C, self).__init__(parent)         
        #self.setWindowIcon(QIcon("res/ico/settingPath.ico"))         
        self.resize(700, 320)
        self.setWindowTitle(u"准则层B3-准则层C")
        self.initUI()
        self.center()        
    ...
    def initUI(self):
        ...
        self.table.setHorizontalHeaderLabels([u"企业性质（C10）", u"所在行业（C11）", u"企业类型（C12）", u"年利润（C13）"])
        self.table.setVerticalHeaderLabels([u"企业性质（C10）", u"所在行业（C11）", u"企业类型（C12）", u"年利润（C13）", u"重要性权重（WCij）", u"一致性指标（CI）", u"平均随机一致性指标（RI）", u"一致性比例（CR）"])
        ...
        for i in range(0,4):
            for j in range(0,4):
                if(self.table.item(i, j).text()):
                    ...
                    break                
        if(flag):
            ReadWrite.Write('Log/DataB3toC.txt',ListOfStr)
            self.close()
            core.SubWeightFig()
